  ...
  for {
    ...
    case newState := <-r.changingState:
      ...
      case START:
        ...
        r.players[0].conn.WriteJSON(protocol.SendPlayTurn(r.boardGame, r.turnsPlayed, r.capturedPawns, -1))
        r.players[1].conn.WriteJSON(protocol.SendRefresh(r.boardGame, r.turnsPlayed, r.capturedPawns))
        log.Println("Starting Game in a room.")
      case RECONNECTED:
        if r.players[0] != nil {
          r.players[0].conn.WriteJSON(protocol.SendStartOfGame(0))
          r.players[0].conn.WriteJSON(protocol.SendRefresh(r.boardGame, r.turnsPlayed, r.capturedPawns))
        }
        if r.players[1] != nil {
          r.players[1].conn.WriteJSON(protocol.SendStartOfGame(1))
          r.players[1].conn.WriteJSON(protocol.SendRefresh(r.boardGame, r.turnsPlayed, r.capturedPawns))
